...
for k = 1:length(matFiles)
    ...
    for v=1:length(ValArray)
        %creating text file
        Val=ValArray(1,v);
        fileID = fopen(sprintf('%s/%s_%d_true.txt', myFolder , newFileName, Val), 'wt');

        % Perform 5D DCT
        fprintf(1, 'True DCT for %s\n',baseFileName);
        fprintf(fileID, 'True DCT for %s\n',baseFileName);
        tic;
        ...
        save(sprintf('%s/%s_%d_true_compressed', newFolder, newFileName, Val), 'A_compressed');

        s=dir(sprintf('%s/%s_%d_true_compressed.mat', newFolder , newFileName, Val));
